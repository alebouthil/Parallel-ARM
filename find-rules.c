...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    if (rank == 0)
      fprintf(
          stderr,
          "Usage: %s <inputfile> <support_threshold> <confidence_threshold>\n",
          argv[0]);
    MPI_Finalize();
    return 1;
  }
  ...
  ItemSet *frequent_pairs = prune_triangle(
      local_tri, local_support, local_transaction_count, &valid_pairs);
  ...
  if (rank == 0 && total_all_itemsets > 0) {
    ...
    for (int src = 0; src < size; src++) {
      for (int i = 0; i < local_itemset_counts[src]; i++) {
        ...
        MPI_Recv(global_itemsets[current_idx].elements, size_buf, MPI_INT, src,
