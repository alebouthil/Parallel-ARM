...
ItemSet *prune_triangle(TriangularMatrix *matrix, float support, int baskets,
  ...
  float support_threshold_count = support * baskets;
...
void check_pairs(TriangularMatrix *matrix, int *items, int count) {
  ...
  for (int i = 0; i < count - 1; i++) {
    for (int j = i + 1; j < count; j++) {
      ...
      int idx_i = (item_i >= 0 && item_i < MAX_ITEM_ID) ? item_to_index[item_i] : -1;
      int idx_j = (item_j >= 0 && item_j < MAX_ITEM_ID) ? item_to_index[item_j] : -1;
