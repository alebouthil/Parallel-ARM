...
ItemSet *prune_triangle(TriangularMatrix *matrix, float local_support,
                        int baskets, int *out_count) {
...
AssociationRule *generate_rules(ItemSet *itemsets, int itemset_count,
  ...
  *rule_count = 0;
  ...
  if (*rule_count > 0) {
    rules = (AssociationRule *)realloc(rules,
                                       (*rule_count) * sizeof(AssociationRule));
  }
